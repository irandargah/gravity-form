...
class GFPersian_Chart_IranDargah
{
    ...
    public static function stats_page()
    {
        ...
        <style type="text/css">
            ...
            .irandargah_summary_value {
                font-size: 20px;
                margin: 5px 0;
            }

            .irandargah_summary_title {
                height: 40px;
            }
...
switch (rgget("tab")) {
        ...
        $sales_label = __("تعداد کل پرداخت های  ایران درگاه این فرم", "gravityformsirandargah");

        $transaction_totals = GFPersian_DB_IranDargah::get_transaction_totals($form_id);
        ...
        ?>
                <div class="irandargah_summary_container">
                    <div class="irandargah_summary_item">
                        <div
                                class="irandargah_summary_title"><?php _e('جمع پرداخت های  ایران درگاه این فرم', 'gravityformsirandargah')?></div>
                        <div
        ...
        $transaction_totals = GFPersian_DB_IranDargah::get_transaction_totals_gateways($form_id);
        $total_sales = empty($transaction_totals["active"]["transactions"]) ? 0 : $transaction_totals["active"]["transactions"];
        $total_revenue = empty($transaction_totals["active"]["revenue"]) ? 0 : $transaction_totals["active"]["revenue"];
        ?>
            ...
            <hr>
            <div class="clear"></div>
            <h2><?php _e(" کل درآمد های ایران درگاه", "gravityformsirandargah")?></h2>
            <div>
        ...
        $transaction_totals = GFPersian_DB_IranDargah::get_transaction_totals_this_gateway();
        $total_sales = empty($transaction_totals["active"]["transactions"]) ? 0 : $transaction_totals["active"]["transactions"];
        $total_revenue = empty($transaction_totals["active"]["revenue"]) ? 0 : $transaction_totals["active"]["revenue"];
        ?>
                <div class="irandargah_summary_container">
                    <div class="irandargah_summary_item">
                        <div
                                class="irandargah_summary_title"><?php _e("جمع پرداخت های  همه فرمهای ایران درگاه", "gravityformsirandargah")?></div>
                        <div
                                class="irandargah_summary_value"><?php echo GF_tr_num(GFCommon::to_money($total_revenue), 'fa') ?></div>
                    </div>
        ...
        $transaction_totals = GFPersian_DB_IranDargah::get_transaction_totals_site();
        $total_sales = empty($transaction_totals["active"]["transactions"]) ? 0 : $transaction_totals["active"]["transactions"];
        $total_revenue = empty($transaction_totals["active"]["revenue"]) ? 0 : $transaction_totals["active"]["revenue"];
        ?>
                <div class="irandargah_summary_container">
    ...
    public static function lastxdays_chart_info($form_id, $chart, $x)
    {
        ...
        if ($chart == 1) {
            ...
            $t = __('ایران درگاه این فرم', 'gravityformsirandargah');
            $results = $wpdb->get_results("SELECT CONVERT_TZ(l.payment_date, '+00:00', '{$tz_offset}') as date, sum(l.payment_amount) as amount_sold, count(l.id) as new_sales
                                        FROM " . GFPersian_DB_IranDargah::get_entry_table_name() . " l
        ...
        if ($chart == 3) {
            $c = 'orang';
            $dt = "}";
            $t = __("همه فرمهای ایران درگاه", 'gravityformsirandargah');
            $results = $wpdb->get_results("SELECT CONVERT_TZ(l.payment_date, '+00:00', '{$tz_offset}') as date, sum(l.payment_amount) as amount_sold, count(l.id) as new_sales
                                        FROM " . GFPersian_DB_IranDargah::get_entry_table_name() . " l

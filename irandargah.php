...
register_activation_hook(__FILE__, array('GFPersian_Gateway_IranDargah', "add_permissions"));
add_action('init', array('GFPersian_Gateway_IranDargah', 'init'));

require_once 'database.php';
require_once 'chart.php';

class GFPersian_Gateway_IranDargah
{
    ...
    {
        $class = 'notice notice-error';
        $message = sprintf(__("آخرین نسخه افزونه را از سایت ایران درگاه دانلود کنید.", "gravityformsirandargah"));
        printf('<div class="%1$s"><p>%2$s</p></div>', $class, $message);
    }
    ...
    public static function gravityformsirandargah($form, $entry)
    {
        $irandargah = array(
            ...
            'title' => __('ایران درگاه', 'gravityformsirandargah'),
            'param' => array(
    ...
    private static function setup()
    {
        if (get_option("gf_irandargah_version") != self::$version) {
            GFPersian_DB_IranDargah::update_table();
    ...
    public static function menu($menus)
    {
        ...
        if (!empty($permission)) {
            $menus[] = array(
                "name" => "gf_irandargah",
                "label" => __("ایران درگاه", "gravityformsirandargah"),
    ...
    public static function uninstall()
    {
        ...
        GFPersian_DB_IranDargah::drop_tables();
    ...
    private static function is_irandargah_page()
    {
        ...
        $current_view = in_array(trim(strtolower(rgget("view"))), array('gf_irandargah', 'irandargah'));
        $current_subview = in_array(trim(strtolower(rgget("subview"))), array('gf_irandargah', 'irandargah'));
    ...
    public static function feed_page()
    {
        ...
        <h3>
            ...
            <a class="add-new-h2"
               href="admin.php?page=gf_irandargah&view=stats&id=<?php echo absint(rgget("id")) ?>"><?php _e("نمودار ها", "gravityformsirandargah")?></a>
        </h3>
    ...
    public static function get_config_by_entry($entry)
    {
        ...
        $feed = !empty($feed_id) ? GFPersian_DB_IranDargah::get_feed($feed_id) : '';
    ...
    public static function get_active_config($form)
    {
        ...
        $configs = GFPersian_DB_IranDargah::get_feed_by_form($form["id"], true);
    ...
    public static function irandargah_page()
    {
        ...
        if ($view == "edit") {
            ...
            GFPersian_Chart_IranDargah::stats_page();
        } else {
            self::list_page('');
        }
    }
    ...
    private static function list_page($arg)
    {
        if (rgpost('action') == "delete") {
            ...
            GFPersian_DB_IranDargah::delete_feed($id);
            ?>
            ...
            if (is_array($selected_feeds)) {
                foreach ($selected_feeds as $feed_id) {
                    GFPersian_DB_IranDargah::delete_feed($feed_id);
                }
            }

            ?>
            <div class="updated fade" style="padding:6px"><?php _e("فید ها حذف شدند", "gravityformsirandargah")?></div>
			<?php
}
        ?>
        <div class="wrap">
			...
			<?php if ($arg != 'per-form') {?>
                ...
                <h2>
					<?php _e("فرم های ایران درگاه", "gravityformsirandargah");
            if (get_option("gf_irandargah_configured")) {?>
                        <a class="add-new-h2"
...
echo '<input type="submit" class="button" value="' . __("اعمال", "gravityformsirandargah") . '" onclick="if( jQuery(\'#bulk_action\').val() == \'delete\' && !confirm(\'' . __("فید حذف شود ؟ ", "gravityformsirandargah") . __("\'Cancel\' برای منصرف شدن, \'OK\' برای حذف کردن", "gravityformsirandargah") . '\')) { return false; } return true;"/>';
        ?>
                        <a class="button button-primary"
                           href="admin.php?page=gf_settings&subview=gf_irandargah"><?php _e('تنظیمات حساب ایران درگاه', 'gravityformsirandargah')?></a>
                    </div>
                </div>
                <table class="wp-list-table widefat fixed striped toplevel_page_gf_edit_forms" cellspacing="0">
					...
					<?php
if ($arg != 'per-form') {
            $settings = GFPersian_DB_IranDargah::get_feeds();
        } else {
            $settings = GFPersian_DB_IranDargah::get_feed_by_form(rgget('id'), false);
        }
                ...
                ?>
                            <tr class='author-self status-inherit' valign="top">
                                ...
                                <td><img style="cursor:pointer;width:25px"
                                         ...
                                         alt="<?php echo $setting["is_active"] ? __("درگاه فعال است", "gravityformsirandargah") : __("درگاه غیر فعال است", "gravityformsirandargah"); ?>"
                                         title="<?php echo $setting["is_active"] ? __("درگاه فعال است", "gravityformsirandargah") : __("درگاه غیر فعال است", "gravityformsirandargah"); ?>"
            ...
            ?>
                        <tr>
                            <td colspan="5" style="padding:20px;">
								<?php
if ($arg == 'per-form') {
                echo sprintf(__("شما هیچ فید ایران درگاهی ندارید . %sیکی بسازید%s .", "gravityformsirandargah"), '<a href="admin.php?page=gf_irandargah&view=edit&fid=' . absint(rgget("id")) . '">', "</a>");
            } else {
                echo sprintf(__("شما هیچ فید ایران درگاهی ندارید . %sیکی بسازید%s .", "gravityformsirandargah"), '<a href="admin.php?page=gf_irandargah&view=edit">', "</a>");
            }
            ?>
                            </td>
                        </tr>
        ...
        <script type="text/javascript">
            ...
            function ToggleActive(img, feed_id) {
                ...
                if (is_active) {
                    ...
                    jQuery(img).attr('title', '<?php _e("درگاه غیر فعال است", "gravityformsirandargah")?>').attr('alt', '<?php _e("درگاه غیر فعال است", "gravityformsirandargah")?>');
                }
                else {
    ...
    public static function update_feed_active()
    {
        ...
        $feed = GFPersian_DB_IranDargah::get_feed($id);
    ...
    private static function get_customer_information_desc($form, $config = null)
    {
        ...
        return self::get_mapped_field_list('irandargah_customer_field_desc', $selected_field, $form_fields);
    }
    ...
    private static function get_customer_information_email($form, $config = null)
    {
        ...
        return self::get_mapped_field_list('irandargah_customer_field_email', $selected_field, $form_fields);
    }
    ...
    private static function get_customer_information_mobile($form, $config = null)
    {
        ...
        return self::get_mapped_field_list('irandargah_customer_field_mobile', $selected_field, $form_fields);
    }
    // ------------------------------------------------------------------------------------------------------------
    ...
    public static function payment_entry_detail($form_id, $entry)
    {
        ...
        if (!empty($payment_gateway) && $payment_gateway == "irandargah") {
            ...
            if (!strtolower(rgpost("save")) || RGForms::post("screen_mode") != "edit") {
                ...
                echo __('درگاه پرداخت : ایران درگاه', 'gravityformsirandargah');
            } else {
                $payment_string = '';
				...
				<?php
echo __('درگاه پرداخت : ایران درگاه (غیر قابل ویرایش)', 'gravityformsirandargah');
            }
    ...
    public static function update_payment_entry($form, $entry_id)
    {
        ...
        $payment_transaction = rgpost("irandargah_transaction_id");
    ...
    public static function settings_page()
    {
        ...
        if (!extension_loaded('soap')) {
            _e('ماژول soap بر روی سرور شما فعال نیست. برای استفاده از درگاه باید آن را فعال نمایید. با مدیر هاست تماس بگیرید.', 'gravityformsirandargah');
        ...
        if (rgpost("uninstall")) {
            ...
            $settings = array(
                "merchent" => rgpost('gf_irandargah_merchent'),
                "server" => rgpost('gf_irandargah_server'),
        ...
        if (!empty($_POST)) {
            ...
            if (isset($_POST["gf_irandargah_configured"]) && ($Response = self::Request('valid_checker', '', '', '')) && $Response != false) {

                if ($Response === true) {
                    echo '<div class="updated fade" style="padding:6px">' . __("ارتباط با درگاه برقرار شد و اطلاعات وارد شده صحیح است .", "gravityformsirandargah") . '</div>';
                } else if ($Response == 'sandbox') {
                    echo '<div class="updated fade" style="padding:6px">' . __("در حالت تستی نیاز به ورود اطلاعات صحیح نمی باشد .", "gravityformsirandargah") . '</div>';
                } else {
        ...
        <form action="" method="post">
            ...
            <table class="form-table">
                ...
                <tr>
                    <th scope="row"><label
                                for="gf_irandargah_merchent"><?php _e("مرچنت", "gravityformsirandargah");?></label></th>
                    <td>
                        <input style="width:350px;text-align:left;direction:ltr !important" type="text"
                               id="gf_irandargah_merchent" name="gf_irandargah_merchent"
        ...
        $gateway_title = __("ایران درگاه", "gravityformsirandargah");
            ...
            ?>
                ...
                <div class="delete-alert alert_red">
                    ...
                    <h3>
						...
						<?php _e("غیر فعالسازی افزونه دروازه پرداخت ایران درگاه", "gravityformsirandargah");?>
                    </h3>

                    <div
                            class="gf_delete_notice"><?php _e("تذکر : بعد از غیرفعالسازی تمامی اطلاعات مربوط به ایران درگاه حذف خواهد شد", "gravityformsirandargah")?></div>

					<?php
$uninstall_button = '<input  style="font-family:tahoma !important;" type="submit" name="uninstall" value="' . __("غیر فعال سازی ایران درگاه", "gravityformsirandargah") . '" class="button" onclick="return confirm(\'' . __("تذکر : بعد از غیرفعالسازی تمامی اطلاعات مربوط به ایران درگاه حذف خواهد شد . آیا همچنان مایل به غیر فعالسازی میباشید؟", "gravityformsirandargah") . '\');"/>';
    ...
    public static function get_gname()
    {
        ...
        if (isset($settings["gname"])) {
            ...
            $gname = __('ایران درگاه', 'gravityformsirandargah');
        }
...
$id = !rgempty("irandargah_setting_id") ? rgpost("irandargah_setting_id") : absint(rgget("id"));
        $config = empty($id) ? array(
            "meta" => array(),
            "is_active" => true,
        ) : GFPersian_DB_IranDargah::get_feed($id);
        $get_feeds = GFPersian_DB_IranDargah::get_feeds();
        $form_name = '';
        ...
        ?>
            ...
            <a class="button add-new-h2" href="admin.php?page=gf_settings&subview=gf_irandargah"
               style="margin:8px 9px;"><?php _e("تنظیمات حساب ایران درگاه", "gravityformsirandargah")?></a>
...
if (!rgempty("gf_irandargah_submit")) {
            ...
            $id = GFPersian_DB_IranDargah::update_feed($id, $config["form_id"], $config["is_active"], $config["meta"]);
            if (!headers_sent()) {
...
$menu_items = apply_filters('gform_toolbar_menu', GFForms::get_toolbar_menu_items($_get_form_id), $_get_form_id);
            echo GFForms::format_toolbar_menu_items($menu_items);?>
                        ...
                        <li class="gf_form_switcher">
							...
							<?php
$feeds = GFPersian_DB_IranDargah::get_feeds();
            if (RG_CURRENT_VIEW != 'entry') {?>
                                <select name="form_switcher" id="form_switcher"
                                        onchange="GF_SwitchForm(jQuery(this).val());">
                                    <option value=""><?php _e('تغییر فید ایران درگاه', 'gravityformsirandargah')?></option>
                            ...
                            <form method="post" action="" id="gform_form_settings">
                                ...
                                <table class="form-table gforms_form_settings" cellspacing="0" cellpadding="0">
                                    ...
                                    <tr style="<?php echo rgget('id') || rgget('fid') ? 'display:none !important' : ''; ?>">
                                        ...
                                        <td>
                                            <select id="gf_irandargah_form" name="gf_irandargah_form"
                                                    onchange="GF_SwitchFid(jQuery(this).val());">
                                                <option
                                                        value=""><?php _e("یک فرم انتخاب نمایید", "gravityformsirandargah");?> </option>
												<?php
$available_forms = GFPersian_DB_IranDargah::get_available_forms();
        ...
        ?>
                                            ...
                                            <img
                                                    ...
                                                    id="irandargah_wait" style="display: none;"/>
                                        </td>
                                    </tr>
                                    ...
                                    <table class="form-table gforms_form_settings"
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <span
                                                        class="description"><?php _e("شورت کد ها : {form_id} , {form_title} , {entry_id}", "gravityformsirandargah");?></span>
                                            </td>
                                        </tr>
...
if (!empty($form)) {
            ...
            ?>
                                        ...
                                        <tr>
                                            <th>
												<?php _e("ایمیل پرداخت کننده", "gravityformsirandargah");?>
                                            </th>
                                            <td class="irandargah_customer_fields_email">
												<?php
if (!empty($form)) {
...
if (!empty($config["meta"]["irandargah_conditional_field_id"])) {
            ...
            foreach ($condition_field_ids as $i => $value): ?>
                                                                    ...
                                                                    <div class="gf_irandargah_conditional_div"
                                                                        ...
                                                                        <select class="gf_irandargah_conditional_operator"
                                                                            ...
                                                                            <option value="contains"><?php _e("شامل میشود", "gravityformsirandargah")?></option>
                                                                            <option value="starts_with"><?php _e("شروع می شود با", "gravityformsirandargah")?></option>
                                                                            <option value="ends_with"><?php _e("تمام میشود با", "gravityformsirandargah")?></option>
                                                                        </select>
        ...
        <style type="text/css">
            ...
            #gf_irandargah_conditional_container *, .delete_this_condition *, .add_new_condition * {
                outline: none !important;
            }
    ...
    public static function Request($confirmation, $form, $entry, $ajax)
    {
        ...
        if (!$valid_checker) {
            ...
            if (!$custom) {
                ...
                gform_update_meta($entry['id'], 'irandargah_feed_id', $config['id']);
            ...
            $ResNumber = apply_filters('gf_irandargah_res_number', apply_filters('gf_gateway_res_number', $entry_id, $entry, $form), $entry, $form);
        } else {
            ...
            $Description = __('جهت بررسی صحیح بودن تنظیمات درگاه گرویتی فرم ایران درگاه', 'gravityformsirandargah');
        ...
        try {
            ...
            $client = new SoapClient('https://dargaah.com/wsdl', array('encoding' => 'UTF-8'));
            ...
            if ($Result->status == 200) {
                ...
                $Payment_URL = 'https://www.dargaah.com/ird/startpay/' . $Result->authority;
        ...
        $confirmation = __('متاسفانه نمیتوانیم به درگاه متصل شویم. علت : ', 'gravityformsirandargah') . $Message;
    ...
    public static function Verify()
    {
        ...
        if (isset($entry["payment_method"]) && $entry["payment_method"] == 'irandargah') {
            ...
            if (!empty($entry["payment_date"])) {
                ...
                $cssClass       = rgar( $form, 'cssClass' );
                $confirmation = __('نتیجه تراکنش قبلا مشخص شده است.' , 'gravityformsirandargah');
            ...
            if ($Status == 'completed') {
                ...
                if ($free == true) {
                    ...
                    $Note = sprintf(__('وضعیت پرداخت : رایگان - بدون نیاز به درگاه پرداخت', "gravityformsirandargah"));
                } else {
                    $Note = sprintf(__('وضعیت پرداخت : موفق - مبلغ پرداختی : %s - کد تراکنش : %s', "gravityformsirandargah"), $Total_Money, $transaction_id);
                }
                ...
                $Note = sprintf(__('وضعیت پرداخت : منصرف شده - مبلغ قابل پرداخت : %s - کد تراکنش : %s', "gravityformsirandargah"), $Total_Money, $transaction_id);
            } else {
                $entry["payment_status"] = "Failed";
                $entry["payment_amount"] = 0;
                $entry["is_fulfilled"] = 0;
                ...
                $Note = sprintf(__('وضعیت پرداخت : ناموفق - مبلغ قابل پرداخت : %s - کد تراکنش : %s - علت خطا : %s', "gravityformsirandargah"), $Total_Money, $transaction_id, $Message);
            }
    ...
    private static function Fault($err_code)
    {
        ...
        switch ($err_code) {
            ...
            case '-1':
                $message = __('اطلاعات ارسال شده ناقص است .', 'gravityformsirandargah');
                break;

            case '-2':
                $message = __('آی پی یا مرچنت ایران درگاه اشتباه است .', 'gravityformsirandargah');
                break;

            case '-3':
                $message = __('با توجه به محدودیت های شاپرک امکان پرداخت با رقم درخواست شده میسر نمیباشد .', 'gravityformsirandargah');
                break;

            case '-4':
                $message = __('سطح تایید پذیرنده پایین تر از سطح نقره ای میباشد .', 'gravityformsirandargah');
                break;

            case '-11':
                $message = __('درخواست مورد نظر یافت نشد .', 'gravityformsirandargah');
                break;

            case '-21':
                $message = __('هیچ نوع عملیات مالی برای این تراکنش یافت نشد .', 'gravityformsirandargah');
                break;

            case '-22':
                $message = __('تراکنش نا موفق میباشد .', 'gravityformsirandargah');
                break;

            case '-33':
                $message = __('رقم تراکنش با رقم وارد شده مطابقت ندارد .', 'gravityformsirandargah');
                break;

            case '-40':
                $message = __('اجازه دسترسی به متد مورد نظر وجود ندارد .', 'gravityformsirandargah');
                break;

            case '-54':
                $message = __('درخواست مورد نظر آرشیو شده است .', 'gravityformsirandargah');
                break;

            case '100':
                $message = __('اتصال با ایران درگاه به خوبی برقرار شد و همه چیز صحیح است .', 'gravityformsirandargah');
                break;

            case '101':
                $message = __('تراکنش با موفقیت به پایان رسیده بود و تاییدیه آن نیز انجام شده بود .', 'gravityformsirandargah');
                break;
